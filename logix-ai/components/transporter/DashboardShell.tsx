...
const NAV_ITEMS = [
  { href: "/transporter/dashboard", label: "Dashboard", icon: LayoutDashboard, exact: true },
  { href: "/transporter/dashboard/shipments", label: "Available Loads", icon: PackageSearch },
  { href: "/transporter/dashboard/bids", label: "Active Bids", icon: Gavel },
  { href: "/transporter/dashboard/trips", label: "My Trips", icon: RouteIcon },
  { href: "/transporter/dashboard/earnings", label: "Earnings", icon: TrendingUp },
  { href: "/transporter/dashboard/demand", label: "Demand Forecast", icon: Compass },
  { href: "/transporter/dashboard/profile", label: "Profile", icon: UserCircle },
];

export function DashboardShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const router = useRouter();
  const { profile, logout } = useTransporter();
        ...
        <Container className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden text-right sm:block">
              ...
              <p className="text-xs text-neutral-500">{profile?.vehicleNumber}</p>
            </div>
            <button
              onClick={() => {
                logout();
